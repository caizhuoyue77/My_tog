...
def clean_relations_without_score(string, entity_id, head_relations):
    """
    对结果进行清洗
    """
    pattern = r"{\s*(?P<relation>[^()]+)\s+}"
    relations = []
...
def construct_relation_prune_prompt(question, entity_name, total_relations, args):
    # TODO:为了debug乱改的，记得要改回来
    total_relations = set(["relation.weather","relation.location","relation.beijing","relation.sunrise","relation.hasSibling"])
...
def relation_search_prune(entity_id, entity_name, pre_relations, pre_head, question, args, czy_client):
    # 获取某个实体的所有关系
    relations = czy_client.search_neighbours_head_tail(27)

    # 这里的relationshipType就是"head"或者"tail"
    head_relations = [rel['relationshipType'] for rel in relations['head']]
    tail_relations = [rel['relationshipType'] for rel in relations['tail']]

    logger.debug("Head relations: %s", head_relations)
    logger.debug("Tail relations: %s", tail_relations)
    ...
    result = "1.{relation.DependOn{score:0.9}}\n2.{relation.ParlyDependOn{score:0.9}}"
    
    logger.debug(result)

    # llm的结果
    flag, retrieve_relations_with_scores = clean_relations_without_score(result, entity_id, head_relations) 

    if len(retrieve_relations_with_scores)>0:
        logger.error("Relations found: %s", retrieve_relations_with_scores)
        return retrieve_relations_with_scores
    else:
        logger.error("Nothing found here")
...
def entity_search(node_id, relation, czy_client):
    """
    刚刚已经根据关系筛选过了，现在来选择对应的实体
    只能找对应实体之间关系在relation中的
    """
    entities = czy_client.search_neighbours_with_relation(node_id, relation)

    return entities

def entity_node_id(enetity_name, enetity_type, czy_client):
    return czy_client.find_node_id_by_name_and_type(enetity_name, enetity_type)
...
def entity_prune(total_entities_id, total_relations, total_candidates, total_topic_entities, total_head, total_scores, args, czy_client):
    ...
    tops = [czy_client.query_all("qid2label", entity_id).pop() if (entity_name := czy_client.query_all("qid2label", entity_id)) != "Not Found!" else "Unname_Entity" for entity_id in tops]
